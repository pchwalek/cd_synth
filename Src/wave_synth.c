...
void calcLidarFreq(int16_t* measurement){
	...
	if(freq_lidar_prev == freq_lidar) freq_lidar_step = 0;
		...
		// 		find out how many approximatley how many buffers you will fill until next lidar reading
		...
		if(lidarModeActive){
			for(int i = 0; i < BUFFER_SIZE; i++){
				...
				table_val = waveTable[incrementIndex(freq_inc, freq_ind)];
					// ew... this code gets messy given that the if statement will be checked at 40kHz (LOL)
